...
class PGDS_multi_sample(object):
    ...
    def train(self, iter_all=200, train_flag=True,model_path='./trained_model/'):
        ...
        Pi_mean = self.collection['Pi_sum'] / self.collection['flag']
        if train_flag:
            np.save(model_path+'Theta_mean.npy', Theta_mean)
            np.save(model_path+'Phi_diag_mean.npy', Phi_diag_mean)
            np.save(model_path+'Phi_drug_mean.npy', Phi_drug_mean)
            np.save(model_path+'Phi_procedure_mean.npy', Phi_procedure_mean)
            np.save(model_path+'Pi_mean.npy', Pi_mean)
